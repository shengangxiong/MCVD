...
from pysot.utils.model_load import check_keys, remove_prefix
        ...
        for epoch in range(cfg.max_epoch_t):
            ...
            if (mAP > max_mAP):
                torch.save(model_teacher.state_dict(),
                           "/home/xjz/L2D-master/pretrained_models/model_teacher_%s_%s_%d.pth"
                           % (cfg.model_t, cfg.dataset, cfg.img_size))
                max_mAP = mAP

        # torch.save(model_teacher.state_dict(), "/home/xjz/L2D-master/pretrained_models/model_teacher_%s_%s_%d.pth"
        #            % (cfg.model_t, cfg.dataset, cfg.img_size))

    else:

        model_teacher.load_state_dict(torch.load("/home/xjz/L2D-master/pretrained_models/model_teacher_%s_%s_%d.pth"
                                                 % (cfg.model_t, cfg.dataset, cfg.img_size)), strict=False)
